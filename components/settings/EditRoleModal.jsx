...
export default function EditRoleModal({ isOpen, onClose, role }) {
  ...
  // Fetch dynamic permissions and set initial state
  useEffect(() => {
    if (isOpen && role) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onClose(true); // true means success, please refresh data
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 fade-in" onClick={() => onClose(false)}>
      <div className="bg-white rounded-[32px] w-full max-w-lg shadow-xl border border-slate-100 flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <input 
              ...
              className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500/20 disabled:opacity-60" 
              placeholder="e.g. Project Manager"
              disabled={role.name === 'Super Admin'} 
            />
            {role.name === 'Super Admin' && <p className="text-xs text-orange-500 mt-1">Super Admin name cannot be changed.</p>}
          </div>
        ...
        <footer className="p-6 border-t border-slate-100 flex justify-between items-center bg-slate-50 rounded-b-[32px]">
          {role.name !== 'Super Admin' ? (
             <button onClick={handleDelete} disabled={isLoading} className="flex items-center gap-2 px-4 py-2 font-bold text-red-600 hover:bg-red-50 rounded-xl transition-colors disabled:opacity-50">
