...
export default function UsersTab() {
  const [users, setUsers] = useState(null); // Mulai dari NULL biar pasti nge-render loading dulu
  ...
  const loadUsers = async () => {
    try {
      ...
      // PASTI masuk ke sini dan update array baru
      if (data?.users && Array.isArray(data.users)) {
        setUsers([...data.users]); // Spread syntax maksa React ngebaca ini array baru (re-render)
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {users.map(u => (
                  <tr key={u.id || u.username} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-4">
